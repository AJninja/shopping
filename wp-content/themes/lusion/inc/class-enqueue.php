			...
			if(!is_admin()){
				...
				/* Enqueue Styles & Script */
                    ...
                    if (class_exists('YITH_WCQV')) {
                    	wp_register_style('lusion-quickview-product', get_template_directory_uri() . '/assets/css/quick-view'.esc_html($lusion_suffix).'.css?ver=' . LUSION_THEME_VERSION);
                    ...
                    if (class_exists('WooCommerce') && is_product() || class_exists('WooCommerce') && is_tax('yith_product_brand')){
                        if (is_rtl()) {
                            wp_register_style('lusion-category-product', get_template_directory_uri() . '/assets/css/products_rtl'.esc_html($lusion_suffix).'.css?ver=' . LUSION_THEME_VERSION);
                            wp_enqueue_style('lusion-category-product');
                            wp_register_style('lusion-single-product', get_template_directory_uri() . '/assets/css/single-product-rtl'.esc_html($lusion_suffix).'.css?ver=' . LUSION_THEME_VERSION);
                            wp_enqueue_style('lusion-single-product');
                        }else{
                            wp_register_style('lusion-category-product', get_template_directory_uri() . '/assets/css/products'.esc_html($lusion_suffix).'.css?ver=' . LUSION_THEME_VERSION);
                            wp_enqueue_style('lusion-category-product');
                            wp_register_style('lusion-single-product', get_template_directory_uri() . '/assets/css/single-product'.esc_html($lusion_suffix).'.css?ver=' . LUSION_THEME_VERSION);
                            wp_enqueue_style('lusion-single-product');
							wp_enqueue_script('sticky-kit');
                        }
                    }
                    if ( is_singular('portfolio') ){
                        wp_register_style('lusion-single-portfolio', get_template_directory_uri() . '/assets/css/single-portfolio'.esc_html($lusion_suffix).'.css?ver=' . LUSION_THEME_VERSION);
                        wp_enqueue_style('lusion-single-portfolio');
                    }
                    if ((is_post_type_archive('portfolio') || taxonomy_exists('portfolio_cat')) && !is_front_page()){
                        wp_register_style('lusion-portfolio', get_template_directory_uri() . '/assets/css/portfolio'.esc_html($lusion_suffix).'.css?ver=' . LUSION_THEME_VERSION);
                    ...
                    if(is_page_template('coming-soon.php')){                        
                        wp_register_script('countdown-scripts', get_template_directory_uri() . '/assets/js/jquery.countdown'.esc_html($lusion_suffix).'.js', array('jquery'), LUSION_THEME_VERSION, true);wp_enqueue_script('countdown-scripts');                        
                        wp_register_script('countdown-comingsoon-scripts', get_template_directory_uri() . '/assets/js/un-minify/countdown'.esc_html($lusion_suffix).'.js', array('jquery'), LUSION_THEME_VERSION, true);wp_enqueue_script('countdown-comingsoon-scripts');                        
                        wp_register_style('lusion-comingsoon-pages', get_template_directory_uri() . '/assets/css/coming-soon'.esc_html($lusion_suffix).'.css?ver=' . LUSION_THEME_VERSION);
                    ...
                    if ( is_front_page() && is_home() ) {
                      // Default homepage
						...
						// blog page
                        if (is_rtl()) {
                            wp_register_style('lusion-blog', get_template_directory_uri() . '/assets/css/blog_rtl'.esc_html($lusion_suffix).'.css?ver=' . LUSION_THEME_VERSION);
                            wp_enqueue_style('lusion-blog');
                        }else{
                            wp_register_style('lusion-blog', get_template_directory_uri() . '/assets/css/blog'.esc_html($lusion_suffix).'.css?ver=' . LUSION_THEME_VERSION);
                            wp_enqueue_style('lusion-blog');
                        }
                      ...
                      // static homepage
                        ...
                        if(is_404()) {
                            wp_register_style('lusion-error-pages', get_template_directory_uri() . '/assets/css/error-pages'.esc_html($lusion_suffix).'.css?ver=' . LUSION_THEME_VERSION);
                            wp_enqueue_style('lusion-error-pages');
                            wp_deregister_style('woocommerce-pre-orders-jquery-ui');
                            wp_deregister_style('woocommerce-pre-orders-jquery-ui-theme');      
                        }
                        if ( is_page( 'cart' ) || is_cart() || is_page( 'wishlist' ) || is_page('checkout') || is_checkout() || is_account_page()) {
                            if (is_rtl()) {
                                wp_register_style('lusion-cart', get_template_directory_uri() . '/assets/css/cart_rtl'.esc_html($lusion_suffix).'.css?ver=' . LUSION_THEME_VERSION);
                                wp_enqueue_style('lusion-cart');
                            }else{
                                wp_register_style('lusion-cart', get_template_directory_uri() . '/assets/css/cart'.esc_html($lusion_suffix).'.css?ver=' . LUSION_THEME_VERSION);
                                wp_enqueue_style('lusion-cart');
                            }
                        }
                        if ( is_page('checkout') || is_checkout()) {
                            wp_enqueue_script('sticky-kit');
                            if (is_rtl()) {
                                wp_register_style('lusion-shipping', get_template_directory_uri() . '/assets/css/shipping_rtl'.esc_html($lusion_suffix).'.css?ver=' . LUSION_THEME_VERSION);
                                wp_enqueue_style('lusion-shipping');
                            }else{
                                wp_register_style('lusion-shipping', get_template_directory_uri() . '/assets/css/shipping'.esc_html($lusion_suffix).'.css?ver=' . LUSION_THEME_VERSION);
                                wp_enqueue_style('lusion-shipping');
                            }
                        }
                    }
