		...
		if ($regular_price_min !== $sale_price_min || $regular_price_max !== $sale_price_max && $product->is_on_sale()) {
			...
			if($product->is_type('variable')){
				if ($percentage_price_max > 0 && $percentage_price_min == 0) {
					$sales_html = '<span class="label-product on-sale percentage"><span>' . esc_html__('-', 'lusion') . $percentage_price_max . '%</span></span>';
					echo wp_kses ($sales_html, lusion_allow_html());
				} elseif ($percentage_price_min > 0 && $percentage_price_max == 0) {
					$sales_html = '<span class="label-product on-sale percentage"><span>' . esc_html__('-', 'lusion') . $percentage_price_min . '%</span></span>';
					echo wp_kses ($sales_html, lusion_allow_html());
				} else {
					$sales_html = '<span class="label-product on-sale upto"><span>' . esc_html__('Up to ', 'lusion') . $percentage_price_max .'%</span></span>';
					echo wp_kses ($sales_html, lusion_allow_html());
				}
			}else{
				$sale_price = round( ( ( $product->get_regular_price() - $product->get_sale_price() ) / $product->get_regular_price() ) * 100 );
				$sales_html = '<span class="label-product on-sale percentage"><span>' . esc_html__('-', 'lusion') . $sale_price . '%</span></span>';
				echo wp_kses ($sales_html, lusion_allow_html());
			}
		}
	}else{
